...
{ using SafeMath for uint256;
    ...
    uint public _totalsupply = 300000000 * 10 ** 18; // 300 Million total supply // muliplies dues to decimal precision
    address public owner;                    // Owner of this contract
    uint256 no_of_tokens;
    bool stopped = false;
    uint256 constant priceICO  = 25; // price is in cents
    ...
    uint public priceFactor;
     uint constant public minimumInvestment = 100; //  $100  is minimum minimumInvestment
 uint public min;
   ...
   function FUTURE1COIN(uint256 EtherPriceFactor) public
    {
   ...
   function () public payable atStage(Stages.ICO)
    {
        uint eth = 10e18;
         min = eth.mul(minimumInvestment).div(priceFactor);
        require(msg.value >= min); // minimumInvestment check
        require(!stopped);
        require(now <= ico_enddate );
        no_of_tokens = ((msg.value).mul(priceFactor.mul(100)).div(priceICO));
        transferTokens(msg.sender,no_of_tokens);
    }
   

    function start_ICO(uint256 _ICOcap) public onlyOwner atStage(Stages.NOTSTARTED)
      {
          require(_ICOcap != 0);
          stage = Stages.ICO;
          stopped = false;
          ...
          Transfer(owner, address(this), balances[address(this)]);
